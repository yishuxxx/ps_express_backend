...
sy.queueAutoRefresh = function(){
  ...
  if(state.auto_refresh_queue && state.auto_refresh_queue.length >= 1){
    ...
    var hide_comments = (state.auto_refresh_queue[0][2] !== undefined) ? parseInt(state.auto_refresh_queue[0][2],10) : -1;
...
sy.hideComments = function(state,page_index,post_index,hide_comments){
  ...
  Comments.map((Comment,i)=>{
    if(hide_comments !== -1 && Comment.is_hidden === false && Comment.can_hide === true && i>=hide_comments){
      hide = true;
      FB.api(
    ...
    }else if(Comment.is_hidden === true && hide_comments !== -1 && i < hide_comments){
      hide = false;
      FB.api(
        '/'+Comment.id+'?access_token='+page_access_token+'&is_hidden='+hide,
        'POST',
